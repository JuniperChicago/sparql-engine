  ...
  const { getGraph, TestEnginePlusLoad } = require('../utils.js')
  ...
  describe('LOAD ///////////////////////////////////////////////////', () => {
    ...
    beforeEach(() => {
      ...
      engine = new TestEnginePlusLoad(gA, GRAPH_IRI)
      engine.addNamedGraph(GRAPH_IRI_2, gB)
    })
    
    it('LOAD Loads external graph and inserts into NAMED graph', done => {
      const query = `
    ...
    it('LOAD loads external graph SILENT and inserts into named DEFAULT GRAPH', done => {
      const query = `
    ...
    it('LOAD loads external graph and inserts into default', done => {
      const query = `
